...
class MyOrderController extends GetxController {
   ...
   void onFilterOrderList(OrderFilterEnum filter)async{
    orderFilterEnum = filter;
     update();
    //dummy
      List<OrderListModel> list = [...orderList.data?.map((value)=> value.copyWith(deliveryStatus: DeliveryStatus.values[filter.index<3? filter.index : 2])).toList() ?? [] ];
      orderList =  orderList.copyWith(requestStatus: RequestStatus.requesting, data: []);
      orderList = orderList.copyWith(requestStatus: RequestStatus.success, data: list);
      update();
   }
