...
class ChatScreen extends StatelessWidget {
  ChatScreen({super.key}): action = (Get.arguments == null? {} : Get.arguments as Map<String, dynamic>)['action'], chatIemWidth = (Utils.deviceSize.width - 32) * .7 ;
  final Widget? action;
        ...
        body: Padding(
          ...
          child: Column(children: [
            Expanded(child: SingleChildScrollView(
              ...
              child: Column(
                children: chatController.chats.map( (item) => _chatItem(item) ).toList()),
            )),
            SizedBox(
              child: Card(
                child: Padding(
                  ...
                  child: Row(children: [
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), color: theme.colorScheme.onSecondary),
                        child: Icon(Icons.send, color: theme.textTheme.bodyLarge?.color,),
                      ),
                    )
                  ]),
                ),
  ...
  Widget _inputField(ChatController chatController) {
    return chatController.selectedImagesPath != null? Stack(
      children: [
         Image.file(height: 150,
                    File(chatController.selectedImagesPath!)),
        Positioned(
          left: 5,
          child: IconButton(onPressed: chatController.onCancelImage, icon: Icon(Icons.cancel_outlined, size: 30,))),
      ],
    )  : CommonTextField( controller: chatController.inputMessageTextFiled,prefixIcon: GestureDetector(
                      onTap: chatController.pickImage,
                      child: Icon(Icons.attach_file)), borderColor: theme.colorScheme.onSecondary);
  }

  Widget _chatItem(ChatModel model){
    bool isMe = model.userInfo.userId == LocalStorage.userId;
    return Align(alignment: isMe? Alignment.centerRight : Alignment.centerLeft,
     child:  model.chatType == ChatType.image?
              SizedBox(
                width: chatIemWidth,
                child: Padding(
      ...
      : Card(color:  isMe? theme.colorScheme.outlineVariant : theme.dividerColor.withAlpha(20),
     child: SizedBox(
      width: chatIemWidth,
       child: Padding(
         padding: const EdgeInsets.all(10),
         child: Column(children: [
            if(model.chatType == ChatType.message)
              Text( model.content, maxLines: null),
            if(model.chatType == ChatType.callFailed)
            Row(children: [
               Icon( Icons.call_missed_rounded, color: isMe? theme.textTheme.bodySmall?.color :theme.colorScheme.error), 
               CommonText(text: 'Voice Call', color: isMe? theme.textTheme.bodySmall?.color :theme.colorScheme.error), 
            ]),
             if(model.chatType == ChatType.callSuccess)
              Row(children: [
                Icon( isMe? Icons.call_made : Icons.call_received), 
                CommonText(text: 'Voice Call'), 
              ]),
            CommonText(text: Utils.formatDateTime(model.createdAt)).end
         ],),
       ),
    ...
    return AppBar(
        ...
        children: [
          ...
          Column(
            ...
            children: [
            CommonText(text: chatController.chatInfo.chatName, style: theme.textTheme.titleSmall,), 
            CommonText(text: chatController.chatInfo.status), 
          ]),
        ],
      ),  leadingWidth: 40, leading: GestureDetector(
        onTap: () {
          Get.back();
        },
        child: Padding(
